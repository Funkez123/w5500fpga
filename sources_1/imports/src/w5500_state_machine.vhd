...
architecture behavioral of w5500_state_machine is
    ...
    signal spi_payload_data_buffer : std_logic_vector(31 downto 0) := (others => '0');    
    signal shift_payload_buffer : std_logic_vector(31 downto 0) := (others => '0'); -- buffer for second process
    signal spi_payload_data_length : integer range 0 to 1514; -- this has to be set every time the raw_payload_buffer is updated.
    ...
    -- led to check if socket is alive
...
begin
	...
	process (clk, reset, w5500state, spi_busy, spi_header_data)
	begin
	
	if clk'event and clk = '1' then
        case w5500state is
            when RESET_STATE =>
                if(tx_payload_ready = '1') then --when the FIFOs in the W5500 Streamer are ready, then we can continue
                ...
                if(spi_busy = '0' and prev_spi_busy = '1') then -- only change states when the spi transmission is done.
                    ...
                    if(w5500state_next /= SET_TX_BUFFER_SIZE_TO_2KB) then -- this prevents setting data again when changing states
                        if(payload_data_has_been_set = '0') then
                            ...
                            spi_payload_data_length <= 1;
                            spi_header_data <= x"0000" & "00000" & '1' & "00";  -- Mode Register 0x0000 -- + "00000" (BSB for common register) + '1' (write) + "00" for vdm
                            spi_payload_data_buffer <= x"80000000"; -- first byte is "10000000" which means the software reset bit is high          
                        end if;
                    end if;
                end if;
                ...
                if(spi_busy = '0' and prev_spi_busy = '1') then -- only change states when the spi transmission is done.
                    ...
                    if(w5500state_next /= SET_GATEWAY_STATE) then -- this prevents setting data again when changing states
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 1;
                            spi_header_data <= x"001F" & "00001" & '1' & "00"; 
                            spi_payload_data_buffer <= x"02000000"; -- 16*1024 =    
                        end if;
                    end if;
                end if;
                ...
                if(spi_busy = '0' and prev_spi_busy = '1') then
                    ...
                    if(w5500state_next /= SET_MAC_STATE_1) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 4;
                            spi_header_data <= x"0005" & "00000" & '1' & "00"; --subnet mask register 0x0005 + WriteCommand in VDM
                            spi_payload_data_buffer <= x"FFFFFF00"; -- 255 255 255 0
                        end if;
                    end if;
                ...
                if(spi_busy = '0' and prev_spi_busy = '1') then
                    ...
                    if(w5500state_next /= SET_MAC_STATE_2) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 4;
                            spi_header_data <= x"0009" & "00000" & '1' & "00"; --source mac address register 0x0009 + WriteCommand 
                            spi_payload_data_buffer <= x"D47F39AE"; -- 0xD4, 0x7F, 0x39, 0xAE (4Bytes)
                        end if;
                    end if; 
                end if;
            ...
            when SET_MAC_STATE_2 => 
                ...
                if(spi_busy = '0' and prev_spi_busy = '1') then
                    ...
                    if(w5500state_next /= SET_SOURCE_IPADDRESS) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 2;
                            spi_header_data <= x"000D" & "00000" & '1' & "00"; --source mac address register 0x0009 (plus 4 Byte offset = 0x000D)
                            spi_payload_data_buffer <= x"92B10000"; -- 0x92, 0xB1 (2 Bytes)
                        end if;
                    end if; 
                end if;
                ...
                if(spi_busy = '0' and prev_spi_busy = '1') then
                    ...
                    if(w5500state_next /= SET_UDP_MODE) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 4;
                            spi_header_data <= x"000F" & "00000" & '1' & "00"; --source ip address register + Write Command in VDM
                            spi_payload_data_buffer <= source_ip_address; 
                        end if;
                    end if; 
                end if;
            ...
            when SET_UDP_MODE =>
                ...
                if(spi_busy = '0' and prev_spi_busy = '1') then
                   ...
                   if(w5500state_next /= SET_SOURCE_SOCKET_PORT) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 1; -- Just one Byte to set Mode
                            spi_header_data <= x"0000" & "00001" & '1' & "00"; --Mode Register : 0x4000 + BSB for Socket 0 Register
                            spi_payload_data_buffer <= x"02000000"; -- first Byte is : "0000 0010" for UDP mode (PDF page 45), enables UDP Multicast and sets UDP mode
                        end if;
                    end if; 
                end if;
            ...
            when SET_SOURCE_SOCKET_PORT =>
                ...
                if(spi_busy = '0' and prev_spi_busy = '1') then
                   ...
                   if(w5500state_next /= OPEN_UDP_SOCKET) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 2; -- Two Bytes to set Socket Port for Socket 0
                            spi_header_data <= x"0004" & "00001" & '1' & "00"; --Socket Port register : 0x0004 + "00001" Socket 0 BSB + Write Command
                            spi_payload_data_buffer <= source_udp_port & x"0000"; --payload provided by source udp port signal 
                        end if;
                    end if; 
                end if; 
                ...
                if(spi_busy = '0' and prev_spi_busy = '1') then
                   ...
                   if(w5500state_next /= CHECK_IF_EXT_TX_AXIS_HAS_DATA) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 1; -- Just one Byte to set Mode
                            spi_header_data <= x"0001" & "00001" & '1' & "00"; --Command Register : 0x0001 + "00001" Socket 0 BSB + Write Command
                            spi_payload_data_buffer <= x"01000000"; -- first Byte is "0x01" (PDF page 46) for Socket OPEN -> S0_SR should change to 0x02 for "open Socket status"
                        end if;
                    end if; 
                end if;
            ...
            when GET_SOCKET_STATUS_THROUGH_SN_SR =>
                if(spi_busy = '0' and prev_spi_busy = '1') then -- if spi is done and the last packa   
                   ...
                   if(w5500state_next /= CHECK_IF_UDP_SOCKET_IS_INITIALIZED) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 1; -- Status is one byte in size
                            spi_header_data <= x"0003" & "00001" & '0' & "00"; --  
                            spi_payload_data_buffer <= x"00000000"; -- 0x0000 because we are just reading
                        end if;
                    end if; 
                end if;
            ...
            when CHECK_IF_UDP_SOCKET_IS_INITIALIZED =>    
                if(rx_payload_last = '1') then
                    if(received_payload_buffer(7 downto 0) = x"00") then --  if socket is closed, then just reopen it
                        w5500state_next <= OPEN_UDP_SOCKET; 
                        led_int <= '0';
                    else 
            ...
            when REQUEST_RECEIVED_DATA_SIZE =>       
                if(spi_busy = '0' and prev_spi_busy = '1') then -- if spi is done and the last packa   
                   ...
                   if(w5500state_next /= WAIT_FOR_REQUESTED_DATA_SIZE) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 2; -- 2 Byte size variable
                            spi_header_data <= x"0026" & "00001" & '0' & "00"; --  
                            spi_payload_data_buffer <= x"00000000"; -- 0x0000 because we are just reading
                        end if;
                    end if; 
                end if;
            ...
            when WAIT_FOR_REQUESTED_DATA_SIZE =>
                if(rx_payload_last = '1') then
                    ...
                    rx_received_size_reg <= received_payload_buffer(15 downto 0);
                end if;   
            ...
            -- reads the RX Read Pointer for Socket 0  
                if(spi_busy = '0' and prev_spi_busy = '1') then
                   ...
                   if(w5500state_next /= WAIT_FOR_RX_READ_POINTER_TO_BE_RECEIVED) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 2; -- Pointer is two bytes
                            spi_header_data <= x"0028" & "00001" & '0' & "00"; --0x28 and 29 are the Sn_RX_RD pointer register, '0' rwb because reading
                            spi_payload_data_buffer <= x"00000000"; -- just reading
                        end if;
                    end if; 
                end if;
            ...
            -- reads the data from the RX Buffer
                if(spi_busy = '0' and prev_spi_busy = '1') then
                   ...
                   if(w5500state_next /= WAIT_FOR_EXT_DATA_HANDLER_TO_FINISH_READING_FROM_FIFO) then
                        if(payload_data_has_been_set = '0') then   
                            spi_header_data <= rx_pointer_reg & "00011" & '0' & "00"; -- pointer that has been read a state before + "BSB for Socket 0 RX Memory" + readCommand '0' + VDM                                
                            spi_payload_data_length <= to_integer(unsigned(rx_received_size_reg)); -- package length that has been read 2 states before 
                            payload_data_has_been_set <= '1';
                        end if;
                    end if; 
                end if;
                ...
                if(spi_busy = '0' and prev_spi_busy = '1') then
                   ...
                   if(w5500state_next /= ISSUE_READ_COMMAND_TO_UPDATE_RX_WRITE_POINTER) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 2; -- Pointer is two bytes
                            spi_header_data <= x"0028" & "00001" & '1' & "00"; -- write new pointer to 0x0028-29 of Socket 0
                            spi_payload_data_buffer <= std_logic_vector(unsigned(rx_pointer_reg)+unsigned(rx_received_size_reg)) & x"0000"; 
                        end if;
                    end if; 
                end if;
            ...
            when ISSUE_READ_COMMAND_TO_UPDATE_RX_WRITE_POINTER =>   
                if(spi_busy = '0' and prev_spi_busy = '1') then
                   ...
                   if(w5500state_next /= GET_SOCKET_STATUS_THROUGH_SN_SR) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 1; -- Command is only one byte
                            spi_header_data <= x"0001" & "00001" & '1' & "00"; -- Command register 0x0001, Socket 0 BSB and write
                            spi_payload_data_buffer <= x"40000000"; -- See Sn_CR , 0x40 completes the read process
            ...
            -- reads the value from the free buffer size register     
                if(spi_busy = '0' and prev_spi_busy = '1') then
                   ...
                   if(w5500state_next /= CHECK_IF_FREE_SIZE_IS_AVAILABLE) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 2; -- 2 Bytes to read
                            spi_header_data <= x"0020" & "00001" & '0' & "00"; -- Free Buffer Size register 0x0020, Socket 0 BSB and read
                            spi_payload_data_buffer <= x"00000000"; -- just reading
                        end if;
                    end if; 
                end if;
            ...
            when CHECK_IF_FREE_SIZE_IS_AVAILABLE =>
                if(rx_payload_last = '1') then
                    ...
                    if(unsigned(received_payload_buffer(15 downto 0)) > 256) then
            ...
            when SET_DEST_IP =>
                ...
                if(spi_busy = '0' and prev_spi_busy = '1') then
                   ...
                   if(w5500state_next /= SET_DEST_UDP_PORT) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 4; -- 4 Bytes to set Dest IP
                            spi_header_data <= x"000C" & "00001" & '1' & "00"; --DEST IP register : 0x000C + "00001" Socket 0 BSB + Write Command
                            spi_payload_data_buffer <= dest_ip_address; -- provided by dest_ip_signal
                        end if;
                    end if; 
                end if;
                ...
                if(spi_busy = '0' and prev_spi_busy = '1') then
                   ...
                   if(w5500state_next /= GET_TX_WR_POINTER) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 2; -- Two Bytes to set Socket Port for Socket 0
                            spi_header_data <= x"0010" & "00001" & '1' & "00"; --DEST port register : 0x0010 + "00001" Socket 0 BSB + Write Command
                            spi_payload_data_buffer <= dest_udp_port & x"0000"; -- (page 30)
                        end if;
                    end if; 
                end if;
            ...
            -- reads the current value of the socket 0 tx buffer pointer             
                if(spi_busy = '0' and prev_spi_busy = '1') then
                   ...
                   if(w5500state_next /= WAIT_FOR_TX_WRITE_POINTER_TO_BE_RECEIVED) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 2; -- Pointer is two bytes in length
                            spi_header_data <= x"0024" & "00001" & '0' & "00"; --  0x24 and 25 are the TX Write pointers for socket 0
                            spi_payload_data_buffer <= x"00000000"; -- 0x0000 because we are just reading
                        end if;
                    end if; 
                end if;
            ...
            -- we let the external source write data into the Socket 0's TX Buffer, this state only set's the spi header and the PASSTHROUGH MODE      
                if(spi_busy = '0' and prev_spi_busy = '1') then
                    ...
                    requested_streammanager_state <= "00"; -- request CONTROLLER_PHASE from the streammanager
                    payload_data_has_been_set <= '0';
                else
                    if(w5500state_next /= UPDATE_TX_WRITE_POINTER_AFTER_WRITE) then
                            spi_header_data <= tx_write_pointer & "00010" & '1' & "00"; -- offset address has been read into rx_shift_payload_buffer in the state before, write to Socket 0: TX-Buffer block
                ...
                if(spi_busy = '0' and prev_spi_busy = '1') then
                   ...
                   if(w5500state_next /= ISSUE_SEND_COMMAND) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 2; -- Pointer is two bytes in length
                            spi_header_data <= x"0024" & "00001" & '1' & "00"; --  0x0024 and 25 are the TX Pointer Registers + "00001" BSB for Socket 0, write command with '1' as rwb bit
                            spi_payload_data_buffer <= std_logic_vector((unsigned(tx_write_pointer) + ptm_transmitted_byte_counter)) & x"0000";   
                        end if;
                    end if; 
                end if;
            ...
            -- allow transmission of that data             
                if(spi_busy = '0' and prev_spi_busy = '1') then
                   ...
                   if(w5500state_next /= REQUEST_INTERRUPT_REG) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 1; -- Just one Byte to read status
                            spi_header_data <= x"0001" & "00001" & '1' & "00"; --  0x0001 for Socket Command Register + "00001" BSB for Socket 0, write command
                            spi_payload_data_buffer <= x"20000000"; -- 0x20, send command
                            tx_write_pointer <= "0000000000000000";
                            ptm_transmitted_byte_counter <= 0;
                        end if;
                    end if; 
                ...
                if(spi_busy = '0' and prev_spi_busy = '1') then
                   ...
                   if(w5500state_next /= WAIT_FOR_INTERRUPT_REGISTER_TO_BE_RECEIVED) then
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 1; -- Just one Byte to read status
                            spi_header_data <= x"0002" & "00001" & '0' & "00";
                            spi_payload_data_buffer <= x"00000000"; -- 0x00, were not writing anything to mosi just reading
                        end if;
                    end if; 
                end if;
                ...
                if(spi_busy = '0' and prev_spi_busy = '1') then
                   ...
                   if(w5500state_next /= GET_SOCKET_STATUS_THROUGH_SN_SR) then 
                        if(payload_data_has_been_set = '0') then
                            payload_data_has_been_set <= '1';
                            spi_payload_data_length <= 1; -- Just one Byte to read status
                            spi_header_data <= x"0003" & "00001" & '1' & "00"; 
                            spi_payload_data_buffer <= received_payload_buffer(7 downto 0) & x"000000"; -- We clear the Interrupt register FLAGS by writing back the ones that are '1', since we have read them in the last state, they are in rx_shift_
                        end if;
                    end if; 
                end if;
...
u_w5500_stream_manager : w5500_stream_manager
 port map(
    ...
    spi_data_buffer => spi_payload_data_buffer,
    spi_data_length => spi_payload_data_length,
    payload_data_has_been_set => payload_data_has_been_set,
